...
const same = (field, a, b) => (field.type === "number" ? isNum(a) && isNum(b) && Math.abs(a - b) < 1e-9 : normKey(a) === normKey(b));
...
export function priceByConfig({ config, stats, xray, listings = {}, choice = {}, inputs = {}, th = {} } = {}) {
  ...
  const fields = (config.schema?.fields || []).filter((f) => f.type !== "text").map((f) => {
    ...
    const value = raw === null || raw === undefined || raw === "" ? null : f.type === "number" ? Number(raw) : raw;
  ...
  return { fields, selectedCount: selected.length, tier, analogs: analogs.length, market, entry, ceiling, marketEff, leaderPrice: leader?.price ?? null, leaderAsin: leader?.asin ?? null, cogsCeiling, marginMin, contributions, warn, analogAsins: analogs.map((a) => a.asin).slice(0, 50) };
}
