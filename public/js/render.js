...
(function () {
  ...
  function secConfig(A, R, o) {
    ...
    return head + btns + stale + toggle + fieldsHtml + dom + priceBlock(A, R, o) + promoBlock(st.promo) + pies + cfgTable(A, C, o);
  }
  /** Промо по нише (spec 014). */
  function promoBlock(P) {
    if (!P) return "";
    if (!P.known) return `<p class="muted" style="font-size:.8rem;margin-top:.6rem">Промо (купоны, дилы, скидки от List Price, Subscribe & Save) появятся после повторной загрузки страниц: текущий кэш сохранён без данных о промо.</p>`;
