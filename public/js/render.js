...
(function () {
  ...
  function secEconomics(A, R, o) {
    ...
    return `<h2>Экономика — Gate 1 / Gate 2 <span class="chip ${g1.status === "pass" ? "ok" : g1.status === "rework" ? "warn" : "fail"}">Gate 1 ${STATUS_LABEL[g1.status]}</span><span class="chip ${g2.status === "pass" ? "ok" : g2.status === "rework" ? "warn" : g2.status === "pending" ? "pending" : "fail"}">Gate 2 ${STATUS_LABEL[g2.status]}</span></h2>
      ...
      <div class="cards">
        ...
        <div class="card ${e.roiHint === "ok" ? "ok" : e.roiHint === "suspicious" || e.roiHint === "low" ? "warn" : "fail"}"><h4>ROI без рекламы</h4><div class="big">${fmtPct(e.roi)}</div><div>прибыль без рекламы ÷ полная себестоимость${isNum(e.criterion2?.["2j"]?.value) ? ` · с рекламой: <b>${fmtPct(e.criterion2["2j"].value, 1)}</b> (2j)` : ""}</div>
  ...
  const NAMES2 = { "2a": "Цена продажи", "2b": "COGS", "2c": "Конверсия", "2d": "Bid/CPC", "2e": "PPC/органика", "2f": "Чистая прибыль/юнит", "2g": "Всего потрачено (реклама)", "2h": "Revenue", "2i": "Всего прибыли", "2j": "% ROI с рекламой", "2k": "% Маржинальность с рекламой" };
  ...
  /** Релевантность патента: модель иногда отдаёт проценты (95) вместо доли (0.95) — старые сканы показывали «9 500 %». */
  const relShare = (v) => { const n = Number(v); if (!isNum(n) || n < 0) return null; return Math.min(1, n > 1 ? n / 100 : n); };
  function secPatents(A, R, o) {
    const P = A.patents;
    const btn = o.static ? "" : `<div class="row noprint" style="margin:.5rem 0"><button data-action="patents" style="flex:0 0 auto">${P ? "🔄 Повторить патентный скан" : "🔎 Патентный скан (AI + Google Patents)"}</button>${P ? '<button data-action="patents-docx" style="flex:0 0 auto" title="Подробный патентный ландшафт одним файлом: держатели, плотность по узлам, все claims с обходами, белые пятна, запросы">↓ Ландшафт в DOCX</button>' : ""}${o.selectedPatentModel ? `<span class="chip" title="Модель для патентного скана">${esc(o.selectedPatentModel)}</span> <a href="#" data-action="settings" class="muted" style="font-size:.8rem">изменить</a>` : ""}<span class="muted" id="patents-status"></span></div>`;
    ...
    return `<h2>Патенты / FTO (критерий 8, Gate 4) <span class="chip ${cls}">AI-скан: ${label}</span>${o.patentsRunning ? '<span class="chip pending">обновляется…</span>' : ""}</h2>${btn}${o.patentsRunning ? '<div class="notice info" style="margin:.4rem 0">Идёт новый патентный скан — ниже предыдущий результат.</div>' : ""}
      ...
      ${(P.whiteSpaces || []).length ? `<div style="margin-top:.7rem"><h4>Белые пятна — где свободно</h4><ul class="tight">${P.whiteSpaces.map((w) => `<li><b>${esc(w.area)}</b> — ${esc(w.why)}. <span class="muted">${esc(w.howToUse)}</span></li>`).join("")}</ul></div>` : ""}
      ${(P.hotAreas || []).length ? `<div style="margin-top:.5rem"><h4>Плотность патентов по узлам</h4><div class="chips">${P.hotAreas.map((a) => `<span class="chip ${a.density === "high" ? "fail" : a.density === "med" ? "warn" : "ok"} has-tip" data-tip="${esc(a.note)}">${esc(a.area)}: ${a.density === "high" ? "минное поле" : a.density === "med" ? "средне" : "почти свободно"}</span>`).join("")}</div></div>` : ""}
      ${(P.holders || []).length ? `<div style="margin-top:.5rem"><h4>Кто владеет патентами</h4><ul class="tight">${P.holders.map((h) => `<li><b>${esc(h.name)}</b> — ${esc(h.focus)}${(h.patents || []).length ? ` <span class="muted">(${esc(h.patents.join(", "))})</span>` : ""}</li>`).join("")}</ul></div>` : ""}
  ...
  function secAi(A, R, o) {
    ...
    return `<h2>AI-вердикт и рекомендации <span class="chip">${esc(ai.model || "")}</span>${ai.adjustedByRules ? '<span class="chip warn">скорректирован правилами</span>' : ""}${ai.compacted ? '<span class="chip warn has-tip" data-tip="Бесплатный уровень модели отказал запросу целиком, поэтому AI получил сокращённые данные: меньше примеров листингов, запросов и цитат из отзывов. Расчёты, пороги и статусы гейтов не менялись.">данные сокращены</span>' : ""}${o.aiRunning ? '<span class="chip pending">обновляется…</span>' : ""}</h2>${btns}${refreshing}<div style="${o.aiRunning ? "opacity:.45;pointer-events:none" : ""}">
  ...
  function secChecklist(A, R) {
    ...
    return `<h2>Чеклист рисков и compliance</h2><div class="tablewrap"><table><tbody>
      ...
      <tr><td>Сертификаты</td><td>${esc(c.certificates || "—")}</td><td>Amazon продаёт сам</td><td>${yes(R.competition.amazonSells)} <small class="muted">${R.competition.amazonSellsSource === "user" ? "вручную" : R.competition.amazonSellsSource === "xray" ? "по Xray (Seller / Fulfillment)" : "авто"}${R.amazon ? ` · ${esc(R.amazon.scopeLabel)} · ${R.amazon.mode === "block" ? "однозначно No-Go" : "в общей картине"}` : ""}</small></td></tr>
      ...
      <tr><td>Склейка отзывов</td><td>${yes(c.reviewMergingSuspected)}</td><td>Купоны/дилы</td><td>${(c.couponsDealsSaturation ?? "unknown") !== "unknown" ? esc({ low: "мало", mid: "умеренно", high: "массово" }[c.couponsDealsSaturation] || c.couponsDealsSaturation) : R.config?.promo?.saturation ? `${esc({ low: "мало", mid: "умеренно", high: "массово" }[R.config.promo.saturation])} <small class="muted">авто по промо листингов</small>` : "—"}</td></tr>
  ...
  const TZ_SECTIONS = ["конструкция", "материалы", "комплектация", "размеры и вес", "функции", "упаковка", "качество и контроль", "сертификация и маркировка", "отличия от конкурентов", "цена и позиционирование"];
  ...
  function secConfig(A, R, o) {
    ...
    const btns = o.static ? "" : `<div class="row noprint" style="margin:.5rem 0;flex-wrap:wrap">
      ...
      ${C.table && promoMissing(A, C) ? `<button data-action="config-promo" ${running ? "disabled" : ""} title="Страницы этих листингов загружены до появления разбора промо: они будут загружены заново (≈ 30 кредитов Scrapfly за страницу), таблица и ручные правки не меняются, AI не вызывается">🏷 Обновить промо · ${fmtN(promoMissing(A, C))} стр.</button>` : ""}
    ...
    const rowsAll = Object.values(C.table.rows || {}); const missing = C.schema.fields.filter((f) => !rowsAll.some((r) => r.values && f.id in r.values));
    const stale = missing.length ? `<div class="notice" style="margin:.5rem 0"><b>Схема изменилась после извлечения:</b> по ${missing.length === C.schema.fields.length ? "всем полям" : `полям ${missing.map((f) => "«" + esc(f.name) + "»").join(", ")}`} данных ещё нет. ${o.static ? "Таблицу нужно извлечь заново." : "Нажмите «Извлечь заново» — страницы уже в кэше, кредиты Scrapfly не тратятся, ручные правки клеток сохранятся."}</div>` : "";
  ...
  function priceBlock(A, R, o) {
    const pc = R.priceConfig; if (!pc) return "";
    const sel = pc.fields.map((f) => `<label class="pf"><span>${esc(f.name)}${f.unit ? ` <small class="muted">${esc(f.unit)}</small>` : ""}</span><select data-price-field="${esc(f.id)}" ${o.static ? "disabled" : ""}><option value="" ${f.value === null ? "selected" : ""}>— не важно —</option>${f.values.map((v) => `<option value="${esc(String(v.value))}" ${f.value !== null && String(v.value) === String(f.value) ? "selected" : ""}>${esc(v.label)}${v.premium ? " ★" : ""}${f.dominant !== null && String(v.value) === String(f.dominant) ? " · доминанта" : ""} · ${fmtPct(v.share)}</option>`).join("")}</select></label>`).join("");
    const tierTxt = pc.fallback === "scope" ? "аналогов нет — цена по всем листингам области" : pc.tier === null ? "" : pc.tier === 1 ? "совпали все выбранные поля" : `совпало ≥ ${Math.round(pc.tier * 100)} % полей`;
    const cards = pc.market === null ? `<div class="notice" style="margin-top:.5rem">${esc(pc.warn || "выберите значения полей — и появится цена")}</div>` : `<div class="cards" style="margin-top:.5rem">
      <div class="card"><h4>Рыночная цена</h4><div class="big">${fmtMoney(pc.market, 2)}</div><div class="muted">медиана ${fmtN(pc.analogs)} аналогов, вес — выручка · ${esc(tierTxt)}</div></div>
      <div class="card"><h4>Вход / потолок</h4><div class="big">${fmtMoney(pc.entry, 2)} – ${fmtMoney(pc.ceiling, 2)}</div><div class="muted">P25 — для нового листинга без отзывов · P75 — верх</div></div>
      <div class="card"><h4>С учётом купонов</h4><div class="big">${fmtMoney(pc.marketEff, 2)}</div><div class="muted">${isNum(pc.marketEff) && isNum(pc.market) && pc.marketEff < pc.market - 0.005 ? "аналоги дают купоны — реальная цена ниже тега" : "купонов у аналогов нет (или промо ещё не загружены)"}</div></div>
      <div class="card"><h4>Ориентир себестоимости</h4><div class="big">≤ ${fmtMoney(pc.cogsCeiling, 2)}</div><div class="muted">при марже ${fmtPct(pc.marginMin)}: цена × (1 − referral) − FBA − маржа</div></div></div>
      ${pc.warn ? `<div class="notice" style="margin-top:.4rem">${esc(pc.warn)}</div>` : ""}`;
    const contrib = pc.contributions.length ? `<div class="tablewrap" style="margin-top:.6rem"><table><thead><tr><th>Поле</th><th>Ваше значение</th><th>Доминанта</th><th class="num">Средняя цена значения</th><th class="num">Δ цены к доминанте</th></tr></thead><tbody>${pc.contributions.map((c) => `<tr><td>${esc(c.name)}</td><td>${c.value === null || c.value === undefined ? '<span class="muted">— не важно —</span>' : `${c.premium ? "★ " : ""}${esc(c.value)}${c.isDominant ? ' <span class="chip">доминанта</span>' : ""}`}</td><td>${esc(c.dominant ?? "—")}</td><td class="num">${fmtMoney(c.avgPrice, 2)}</td><td class="num">${c.isDominant ? "—" : isNum(c.delta) ? (c.delta >= 0 ? "+" : "−") + fmtMoney(Math.abs(c.delta), 2) : "—"}</td></tr>`).join("")}</tbody></table></div>` : "";
    const band = R.priceBand; const outside = band?.active && isNum(pc.market) && ((band.min !== null && pc.market < band.min) || (band.max !== null && pc.market > band.max));
    const apply = o.static || !isNum(pc.market) ? "" : `<div class="row noprint" style="margin-top:.5rem;flex-wrap:wrap"><button data-action="price-apply" data-price="${pc.market}">Подставить ${fmtMoney(pc.market, 2)} в экономику</button>${outside ? `<span class="chip warn">вне вашего ценового диапазона ${esc(band.label)}</span>` : ""}${pc.fallback === "scope" ? '<span class="chip warn">это цена по всей области, а не по аналогам</span>' : ""}<span class="muted" style="font-size:.8rem">Ориентир, не формула: бренд и отзывы влияют на цену не меньше конфигурации.</span></div>`;
  ...
  function cfgTable(A, C, o) {
    ...
    const cell = (asin, f, c) => { const v = c?.value; const none = v === null || v === undefined; const txt = none ? "нет данных" : f.type === "number" ? fmtN(v, 2) + (f.unit ? " " + f.unit : "") : String(v);
      const title = [c?.source ? "источник: " + (CFG_SRC[c.source] || c.source) : "", c?.unlisted ? "значение вне списка схемы — добавьте его в список поля («Править схему») или объедините с существующим" : ""].filter(Boolean).join(" · ");
      return `<td class="${none ? "muted" : ""}${c?.unlisted ? " unlisted" : ""}${o.static ? "" : " editable"}"${o.static ? "" : ` data-cell="${esc(asin)}|${esc(f.id)}"`}${title ? ` title="${esc(title)}"` : ""}>${esc(txt)}${c?.unlisted ? " *" : ""}${c?.source === "manual" ? " ✎" : ""}</td>`; };
    ...
    return `<details class="cfgtable" ${o.configTableOpen ? "open" : ""}><summary>Таблица характеристик: ${fmtN(rows.length)} листингов × ${fields.length} полей${failed.length ? ` · не загружено ${failed.length}` : ""}${o.static ? "" : " — клетку можно править"}</summary>
      <div class="tablewrap" style="max-height:60vh;overflow:auto"><table><thead><tr><th>ASIN</th><th>Бренд</th><th class="num">Цена</th><th class="num">Выручка</th><th>Промо</th>${fields.map((f) => `<th title="${esc(f.hint || CFG_TYPE[f.type] || "")}">${esc(f.name)}</th>`).join("")}</tr></thead><tbody>${rows.map(([asin, r]) => { const a = byAsin.get(asin) || {}; return `<tr class="${r.status === "failed" ? "muted" : ""}"><td><a href="https://www.amazon.com/dp/${esc(asin)}" target="_blank" rel="noopener">${esc(asin)}</a>${r.status === "failed" ? '<br><small class="muted">страница не загрузилась</small>' : ""}</td><td>${esc(a.brand || "")}</td><td class="num">${fmtMoney(a.price, 2)}</td><td class="num">${fmtK(revenueOf(a))}</td><td class="promo">${promoChips(A.aggregates.listings?.[asin]?.promo)}</td>${fields.map((f) => cell(asin, f, r.values?.[f.id])).join("")}</tr>`; }).join("")}</tbody></table></div>
      ...
      <p class="muted" style="font-size:.8rem">Извлечено ${fmtDate(C.table.extractedAt)} · модель ${esc(C.table.model || "—")}${o.static ? "" : " · клик по клетке — исправить значение вручную (пометка ✎; повторное извлечение ручные значения не трогает)"}. Пометка * — значение найдено в листинге, но его нет в списке поля: оно учтено в диаграммах своей группой${o.static ? "" : "; добавьте его в список или объедините («Править схему»)"}.</p></details>`;
  }
  ...
  const TIPS = {
    ...
    "config|Средняя цена значения": "Средняя цена листингов ниши с этим значением поля.",
    "config|Δ цены к доминанте": "Насколько средняя цена выбранного значения отличается от средней цены доминирующего: за что покупатели платят больше или меньше.",
  ...
  const KEY_TIPS = {
    ...
    "2c": "Конверсия клика в покупку. Для нового листинга реалистично 8–15 %; выше — OK, только если конверсия ниши по POE или рынка по SQP это подтверждает, иначе «погранично» (оптимистично). Ниже 8 % — погранично.",
    ...
    "2j": "ROI С рекламой за 90 дней: прибыль после рекламы ÷ вложенное (товар + реклама). Порог — 20 %. Обязательный показатель. Отличается от карточки «ROI без рекламы» — там реклама не учтена.",
