...
(function () {
  ...
  function secCompetitors(A, R) {
    ...
    const az = R.amazon; const azChip = az?.present ? `<span class="chip ${az.blocks ? "fail" : "warn"}" title="${esc(az.blocks ? "Порог «Amazon как продавец»: однозначно No-Go. Область: " + az.scopeLabel : "Порог «Amazon как продавец»: учитывается в общей картине (1e, scorecard). Область: " + az.scopeLabel)}${az.note ? ". " + esc(az.note) : ""}">Amazon продаёт сам${az.count ? ` · ${az.count} ASIN` : ""}${isNum(az.revenueShare) ? ` · ${fmtPct(az.revenueShare)} выручки` : ""}${az.blocks ? " · No-Go" : ""}</span>` : "";
    return `<h2>Конкурентная карта${bandChip(R, "band")} <span class="chip ${comp.dominant ? "fail" : "ok"}">${comp.dominant ? "доминирующий бренд" : "без доминации"}</span>${azChip}${comp.amazonSells ? '<span class="chip fail">Amazon продаёт сам</span>' : ""}</h2>
      <div class="tiles">
