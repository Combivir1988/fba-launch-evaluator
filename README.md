...
3. **AI-анализ** (`claude-opus-5` по умолчанию): Claude получает компактный агрегат (не файлы) и возвращает JSON по схеме — вердикт, решающий гейт, обоснование, гипотезы дифференциации из отзывов, рекомендации, шаги. Вердикт AI **не может быть мягче правил**: при конфликте понижается с пометкой.
...
|---|---|---|
| `ANTHROPIC_API_KEY` | ключ Claude (только сервер) | — |
...
Стоимость одного AI-анализа на Opus 5 ≈ $0.08–0.13 (≈ 6–10k входных токенов, системный промпт кэшируется); на `claude-sonnet-5` ≈ $0.04.
