...
export const TZ_SECTIONS = ["конструкция", "материалы", "комплектация", "размеры и вес", "функции", "упаковка", "качество и контроль", "сертификация и маркировка", "отличия от конкурентов", "цена и позиционирование"];
...
export const SYS_TZ = `Ты составляешь техническое задание (ТЗ) производителю для товара Private Label на Amazon FBA по ФАКТАМ, которые передало приложение: доминирующая конфигурация ниши (какие значения характеристик собирают выручку), «премиальные» значения (доля выручки выше доли листингов при цене выше медианы), цены и сегменты, темы негативных и позитивных отзывов ниши, регуляторные триггеры, итог патентного скана, гипотезы дифференциации, размер первой партии, ценовой ориентир (рыночная цена аналогов такой же конфигурации, вход/потолок, ориентир себестоимости для минимальной маржи) и промо в нише (доля выручки с купонами/дилами/скидками).
Правила: 1) База — доминирующая конфигурация: по каждому полю требование с доминирующим значением (priority must), если факты не говорят обратного. 2) Где отличаться — из негативных отзывов и гипотез дифференциации: измеримые требования (should или must, если жалоба массовая). 3) Позитивные темы — «что не сломать». 4) Регуляторные триггеры и патенты — требования к сертификации, маркировке, проверке до заказа (без юридических выводов). 5) КАЖДОЕ число в требовании и обосновании бери ТОЛЬКО из переданных фактов и указывай источник; если факта нет — формулируй без числа или вынеси в openQuestions. 6) Не упоминай бренды-конкуренты в требованиях. 7) 15–40 строк, группируй по разделам схемы. 8) Если переданы pricing/promo — добавь в раздел «цена и позиционирование» целевую розничную цену, ориентир себестоимости и вывод о купонах/дилах (нужен ли запас в цене на промо). ${RULES}`;
...
const SECTION_HINTS = [[/сертиф|маркиров|compliance|fda|ce\b|ul\b|safety|безопас/i, "сертификация и маркировка"], [/упаков|коробк|packag/i, "упаковка"], [/качеств|контрол|aql|тест|провер/i, "качество и контроль"], [/материал|ткан|металл|пластик|latex|silicone|material/i, "материалы"],
  [/комплект|в комплекте|аксессуар|включ|accessor|includ/i, "комплектация"], [/размер|вес|габарит|dimension|weight|size/i, "размеры и вес"], [/функци|режим|feature|function|mode|подключ|bluetooth|app/i, "функции"], [/отлич|конкурент|дифференц|уник|usp|differ/i, "отличия от конкурентов"], [/цен|price|позиционир|pricing|стоимост|маржа|себестоим/i, "цена и позиционирование"]];
