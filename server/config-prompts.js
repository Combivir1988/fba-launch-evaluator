...
Правила: 1) 8–20 полей, только то, что реально встречается в листингах ниши (не общие «Бренд», «Цена», «Рейтинг»). 2) Для каждого поля выбери тип: choice — конечный список вариантов (2–10 значений, объедини синонимы: «Bluetooth», «Bluetooth 5.0», «BT» → одно значение); number — измеримая величина с единицей; text — свободный короткий текст (только когда список невозможен). 3) Названия полей и варианты — по-русски (общепринятые технические термины можно оставить: Bluetooth, USB-C, IPX7). 4) id — латиницей snake_case, уникальный. 5) hint — где это обычно написано в листинге. ${RULES}`;

export const SYS_EXTRACT = `Ты извлекаешь характеристики товаров из текста листингов Amazon по заданной схеме полей.
Правила: 1) Для каждого листинга и КАЖДОГО поля схемы дай значение. 2) Поле choice — только одно значение ТОЧНО из списка вариантов; если в тексте ничего подходящего нет — «нет данных». Не выбирай «наиболее вероятное». 3) Поле number — число как в тексте (можно с единицей: «12.5 lb»); переводить единицы не нужно. 4) Поле text — коротко, дословно по листингу. 5) source — где именно найдено (title / bullets / specs / aplus); для «нет данных» — none. 6) Ничего не додумывай по картинкам, бренду или похожим товарам — только текст, который передан. ${RULES}`;
...
export function fieldsUser({ niche, coreKeyword, listings }) {
  return `Ниша: ${niche || coreKeyword}\nГлавный ключ: ${coreKeyword || "—"}\nЛистингов: ${listings.length}\n\n` + listings.map((l) => listingText(l, { maxBullets: 8, maxSpecs: 20, maxChars: 1600 })).join("\n\n---\n\n");
}
