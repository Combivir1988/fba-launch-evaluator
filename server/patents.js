...
export const ASSESS_SCHEMA = { type: "object", additionalProperties: false, required: ["overall", "summary", "patents", "nextSteps", "designPatentNote"], properties: {
  ...
  // Патентный ландшафт (2026-09-25): кроме списка патентов — кто владеет, где «минное поле» и где свободно.
  holders: { type: "array", items: { type: "object", additionalProperties: false, required: ["name", "focus"], properties: {
    name: { type: "string", description: "Патентообладатель" }, focus: { type: "string", description: "Что держит: технологии, 1-2 предложения" },
    patents: { type: "array", items: { type: "string" }, description: "Номера его патентов из списка" } } } },
  hotAreas: { type: "array", items: { type: "object", additionalProperties: false, required: ["area", "density", "note"], properties: {
    area: { type: "string", description: "Узел или функция товара" }, density: { type: "string", enum: ["high", "med", "low"] },
    note: { type: "string", description: "Кто патентует и что именно" } } } },
  whiteSpaces: { type: "array", items: { type: "object", additionalProperties: false, required: ["area", "why", "howToUse"], properties: {
    area: { type: "string", description: "Свободная ниша: функция, которую почти не патентуют" },
    why: { type: "string", description: "Почему свободно: истёкшие патенты, нет claims, общая техника" },
    howToUse: { type: "string", description: "Как использовать это в нашем товаре" } } } } } };
...
Правила: 1) Оценивай пересечение по НЕЗАВИСИМЫМ claims, а не по названию/абстракту. 2) Истёкшие патенты (expired=true) — риск none, но отметь как prior art. 3) risk=high только если наш дифференциатор/конструкция прямо читается на независимый claim действующего патента; med — частичное совпадение или неясная формулировка; low — далёкая аналогия. 4) overall: conflict — есть хотя бы один high; unsure — есть med или данных мало; clear — только none/low. 5) Не выдумывай патенты — оценивай только переданные. 6) Кроме списка патентов дай картину ландшафта: holders - кто владеет ключевыми патентами и на чём специализируется; hotAreas - узлы товара с плотностью патентов (high - минное поле, med - средне, low - почти свободно); whiteSpaces - 2-5 белых пятен: функции, которые почти не патентуют (истёкшая или общая техника, отсутствие claims), почему свободно и как этим воспользоваться в нашем товаре. 7) В designAround пиши конкретный обход: какой признак независимого claim убрать или заменить, а не «изменить конструкцию». 8) Пиши сухо, кратко, без markdown. Только JSON.`;
...
export async function* patentScanStream(body, cfg, { signal, fetchImpl = fetch, aiJson = openrouterJson } = {}) {
  ...
  try {
    ...
    let assess;
    if (cfg.mock) assess = { overall: "unsure", summary: "[MOCK] Демонстрационная оценка без AI.", designPatentNote: "Проверьте design patents по картинкам лидеров.", patents: packed.map((p, i) => ({ number: p.number, relevance: 0.5, risk: i === 0 ? "med" : "low", claimed: "mock", overlap: "mock", designAround: "mock" })), holders: [{ name: '[MOCK] Держатель', focus: 'фильтры и ножи', patents: packed.slice(0, 1).map((p) => p.number) }], hotAreas: [{ area: 'фильтрация', density: 'high', note: '[MOCK]' }], whiteSpaces: [{ area: 'таймер отложенного старта', why: 'общая техника, claims нет', howToUse: 'можно ставить без лицензии' }], nextSteps: ["Показать патентному поверенному"] };
    ...
    const scan = { createdAt: new Date().toISOString(), status: assess.overall, summary: assess.summary, designPatentNote: assess.designPatentNote, nextSteps: assess.nextSteps || [], items,
      holders: (assess.holders || []).slice(0, 8), hotAreas: (assess.hotAreas || []).slice(0, 8), whiteSpaces: (assess.whiteSpaces || []).slice(0, 6),
