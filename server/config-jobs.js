...
export async function* schemaStream(body, cfg, deps = {}) {
  ...
  try {
    ...
    const raw = cfg.mock ? mockFields({ listings }) : await retryAi(() => aiJson({ cfg, model, system: SYS_FIELDS, user: fieldsUser({ niche: body.niche, coreKeyword: body.coreKeyword, listings, prevSchema: body.prevSchema || null }), schema: FIELDS_SCHEMA, signal: deps.signal, fetchImpl: deps.fetchImpl, maxTokens: 6000 }), { pauseMs: deps.aiPauseMs ?? 15000, signal: deps.signal });
