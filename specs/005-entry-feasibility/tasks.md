...
- [x] T019 Все тесты и пробы 6, 7, 9, 12; коммит, push, деплой при `health.running = 0`, проверка на проде
- [x] T020 Память проекта и внешний CLAUDE.md

## Выпуск

- 2026-09-20: коммит f278e4c на проде (Railway, деплой при `health.running = 0`). Тесты 207, проба `ui-probe12` — 17 проверок, регрессионные пробы 6, 7, 9, 10 зелёные. Проверка на проде через временные публичные ссылки (полная и «без закупочной экономики») — 9 из 9, временные записи удалены.
