...
- 2026-09-25: фильтр «мои» пробовали расширить до «создал ИЛИ менял» — ОТКАЗАНО господарём, вернули как было (коммит-возврат). Причина пустого списка была не в фильтре: администратор вошёл через Google под отдельной учётной записью «Валентин», а анализы создавали Iryna и admin. Поведение по авторству — задуманное: «мои» = созданные мной.
- 2026-09-25: «Настройки → Модели AI» перегружали обычного пользователя. Теперь ему видны только два селекта с короткой подписью «бесплатно / платная» и пояснениями в два слова («вердикт, обоснование и рекомендации», «чтение патентных claims»). Абзац про провайдера, переменную OPENROUTER_MODELS, общий пул и кредиты, подробные подписи моделей (эндпойнт Nvidia, общий пул, нужен счёт) и итоговая строка с ценами показываются только администратору — как и разделы «Пользователи» и «Сеансы». Абзац справки про настройки сокращён вдвое.
