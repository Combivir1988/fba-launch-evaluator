...
- 2026-09-25: фильтр «мои» в истории показывал только созданные мной анализы, поэтому у того, кто дорабатывает чужие, список был пуст (на проде у Валентина 0 своих при 7 анализах команды, все — авторства Iryna/admin/timmycrc, но правил их он). Теперь «мои» = `created_by = я OR updated_by = я`, в списке подпись «мои (создал или менял)», пустой ответ объясняет, что показывает фильтр и предлагает «все». Тест в analyses.test.js: правка чужого анализа переносит его в «мои», у автора он остаётся.
